...
def plot_trajectories_2(Xts, t, remove_time=True, fig_axs=None, color='b', 
    ...
    for i in range(n):
        ...
        axs.plot(t.cpu().flatten(), Xts[i, :, :].detach().cpu().numpy().flatten(), 
                 color, alpha=0.3, label=label)
...
class mocap_data_show(matplotlib_show):
    ...
    def draw_vertices(self):
        self.points_handle = self.axes.scatter(self.vals[:, 0], self.vals[:, 1], 
                                               self.vals[:, 2], color=self.color)
    ...
    def draw_edges(self):
        ...
        if self.connect is not None:
            ...
            for i, j in zip(self.I, self.J):
                ...
                z.append(np.NaN)
            self.line_handle = self.axes.plot(np.array(x), np.array(y), 
                                              np.array(z), '-', color=self.color)
    ...
    def process_values(self):
        raise NotImplementedError("Need to be implement to use data_show class")

    def initialize_axes(self, boundary=0.05):
        """Set up the axes with the right limits and scaling."""
        bs = [(self.vals[:, i].max()-self.vals[:, i].min())*boundary 
              for i in range(3)]
        self.x_lim = np.array([self.vals[:, 0].min() - bs[0], 
                               self.vals[:, 0].max() + bs[0]])
        self.y_lim = np.array([self.vals[:, 1].min() - bs[1], 
                               self.vals[:, 1].max() + bs[1]])
        self.z_lim = np.array([self.vals[:, 2].min() - bs[2], 
                               self.vals[:, 2].max() + bs[2]])
...
#         self.axes.auto_scale_xyz([-1., 1.], [-1., 1.], [-1., 1.])
        ...
        extents = np.array([getattr(self.axes, 'get_{}lim'.format(dim))() 
                            for dim in 'xyz'])
...
class skeleton_show(mocap_data_show):
    """data_show class for visualizing motion capture data encoded as a skeleton 
       with angles.
    """
    
    def __init__(self, vals, skel, axes=None, padding=0, color='b'):
        """data_show class for visualizing motion capture data encoded as a 
           skeleton with angles.
        :param vals: set of modeled angles to use for printing in the axis when 
            it's first created.
        :type vals: np.array
        :param skel: skeleton object that has the parameters of the motion 
            capture skeleton associated with it.
        ...
        super(skeleton_show, self).__init__(vals, axes=axes, connect=connect, 
                                            color=color)

    def process_values(self):
        """Takes a set of angles and converts them to the x,y,z coordinates in 
           the internal prepresentation of the class, ready for plotting.
        :param vals: the values that are being modelled."""

        if self.padding>0:
            channels = np.zeros((self.vals.shape[0], 
                                 self.vals.shape[1] + self.padding))
    ...
    def wrap_around(self, lim, connect):
        quot = lim[1] - lim[0]
        self.vals = np.rem(self.vals, quot) + lim[0]
...
def cmu_mocap(data_Y, data, subject='35', motion=['01'], in_place=True, 
              optimize=True, verbose=True, plot=True, axes=None, time_index=1, 
              camera=None, standardise=True):
    if in_place:
...
def animate_skeleton(Y, data, notebook=True, standardise=False):
    ...
    for i in range(N):    
        cmu_mocap(Y, data, axes=axes, camera=camera, time_index=i, 
                  in_place=False, standardise=standardise)
    return camera
...
def plot_pendulum(Xts, t, P_0=None, P_1=None, axs=None, color="r", alpha=1.0):
    ...
    for dim_j in range(dim):
        ...
        for i in range(Xts.shape[0]):
            axs[dim_j, 1].plot(t, Xts[i, :, dim + dim_j].flatten(), color, 
                               alpha=alpha)
...
def auxiliary_plot_routine_init(Xts, t, prior_X_0, X_0, X_1, drift_backward, 
                                sigma, N, dt, device):
    plot_pendulum(Xts, t,prior_X_0, X_1, color="g", alpha=0.5)
    plt.show()
    T2, M2 = solve_sde_RK(b_drift=drift_backward, sigma=sigma, X0=X_1, dt=dt, 
                          N=N, device=device)
    T3, M3 = solve_sde_RK(b_drift=drift_backward, sigma=sigma, X0=Xts[:, 0, :-1], 
...
def auxiliary_plot_routine_end(Xts, t, prior_X_0, X_0, X_1, drift_forward, 
                               drift_backward, sigma, N, dt, device):
    T2, M2 = solve_sde_RK(b_drift=drift_backward, sigma=sigma, X0=X_1, dt=dt, 
                          N=N, device=device)
    T3, M3 = solve_sde_RK(b_drift=drift_forward, sigma=sigma, X0=X_0, dt=dt, 
                          N=N, device=device)
    print("PLOT")
